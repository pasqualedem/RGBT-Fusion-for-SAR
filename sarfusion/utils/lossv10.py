...
class v8DetectionLoss:
    ...
    def __call__(self, preds, targets):
        ...
        imgsz = (
            torch.tensor(feats[0].shape[2:], device=targets.device, dtype=dtype)
            * self.stride[0]
        )  # image size (h,w)
...
class v10DetectLoss:
    ...
    def __call__(self, preds: ModelOutput, targets):
        ...
        return LossOutput(
            ...
            components=torch.cat((loss_one2many[1], loss_one2one[1])),
        )
        return (
            loss_one2many[0] + loss_one2one[0]
        )  # , torch.cat((loss_one2many[1], loss_one2one[1]))
