...
def plot_images(
    ...
    for i in range(bs):
        ...
        if img.shape[2] == 4: # RGBT
            if ((img[:, :, 0:3] - IGNORE_INDEX) < 1e-5).all(): # IR Image
                img_type = "IR"
                img = img[:, :, 3:].repeat(3, 2)
            else:
                if ((img[:, :, 3] - IGNORE_INDEX) < 1e-5).all(): # RGB Image:
                    img_type = "RGB"
                else:
                    img_type = "RGBT"
                img = img[..., :3] # Only RGB
    ...
    for i in range(bs):
        ...
        if paths:
            ...
            annotator.text((x + 5, y + h - 25), text=f"{img_types[i]}", txt_color=(220, 220, 220))  # image type
        if len(cls) > 0:
            idx = batch_idx == i
