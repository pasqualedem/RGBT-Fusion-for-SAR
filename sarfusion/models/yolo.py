...
from sarfusion.utils.general import (
    LOGGER,
    check_version,
    check_yaml,
    make_divisible,
...
from sarfusion.utils.torch_utils import (
    fuse_conv_and_bn,
    initialize_weights,
    model_info,
    profile,
...
class DualDDetect(nn.Module):
    ...
    def forward(self, x):
        ...
        if self.training:
            return ModelOutput(features=d1, features_aux=d2)
        elif self.dynamic or self.shape != shape:
            self.anchors, self.strides = (
                d1.transpose(0, 1) for d1 in make_anchors(d1, self.stride, 0.5)
            )
...
class DetectionModel(BaseModel):
    ...
    def __init__(
        ...
        if isinstance(m, (Detect, DDetect, Segment, Panoptic)):
            ...
            forward = lambda x: (
                self.forward(x)[0]
                if isinstance(m, (Segment, Panoptic))
                else self.forward(x)
            )
